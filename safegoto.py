...
class Navigator(object):
    def __init__(self, run_mode = 'hw'):
        ...
        if run_mode == 'hw':
            ...
            rospy.Subscriber('/endgoal', String, self.end_goal_update)
        elif run_mode == 'sim':
    ...
    def end_goal_update(self, endgoal):
        self.final_destination = list(map(float, endgoal.data.split()))
    ...
    def calculate_obstacle_potential_field(self):
        ...
        laser_obstacle_vector = self.get_potential_field(laser_range_data,k_o_laser)
        sonar_obstacle_vector = self.get_potential_field(sonar_range_data,k_o_sonar)
    ...
    def navigate(self):
        ...
        print_wait =True   
        while self.final_destination is None:
            if print_wait:
                print("Waiting for final dest")
                print_wait = False
        ...
        while self.get_euclidian_distance(self.final_destination, self.current_location) > 0.50: # position accurate within  50 cm
            self.next_goal = self.planned_goal
